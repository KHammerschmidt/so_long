...
/*   Updated: 2021/10/24 21:47:04 by khammers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_even_nbrs(t_map *game, int p, int t, int s)
{
	...
	if (nbr_rd % 2 == 0)
		...
		t--;
	while ((game->map[t][s] == 'E' || game->map[t][s] == 'C' || game->map[t][s] == 'G') && (flag != 4))
	{
		flag++;
		if (nbr_rd % 2 == 0)
			t++;
...
void	ft_enemy_movement(t_map *game)
{
	...
	while (p < game->counter_enemy)
	{
		...
		s = game->e_pos_x[p];
		if (p % 2 == 0)
			ft_even_numbers(game, p, t, s);
		else
			ft_uneven_numbers(game, p, t, s);
		p++;
	}
}